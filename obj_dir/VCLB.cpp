...
VCLB::VCLB(VerilatedContext* _vcontextp__, const char* _vcname__)
    ...
    , clk{vlSymsp->TOP.clk}
    , cfg_en{vlSymsp->TOP.cfg_en}
    , cfg_in{vlSymsp->TOP.cfg_in}
...
void VCLB___024root__trace_init_top(VCLB___024root* vlSelf, VerilatedFst* tracep);
...
VL_ATTR_COLD void VCLB::trace(VerilatedFstC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'VCLB::trace()' shall not be called after 'VerilatedFstC::open()'.");
    }
